    ...
    if (cluster.isMaster) {
      ...
      prompt.get(promptSchema, function (err, result) {
        ...
        try {
          ...
          fs.statSync(`${result.path}FASTFormGenre.nt`)
        ...
        var files = [`${result.path}FASTChronological.nt`, `${result.path}FASTFormGenre.nt`, `${result.path}FASTCorporate.nt`, `${result.path}FASTEvent.nt`, `${result.path}FASTGeographic.nt`, `${result.path}FASTPersonal.nt`, `${result.path}FASTTopical.nt`, `${result.path}FASTTermEvent.nt`]

        // to test:
        // files = [`${result.path}FASTTermEvent.nt`]
      ...
      process.on('message', (msg) => {
        ...
        // we are being tricky and loading the Event file into two differnt processes but the same file name
        var inputStream = fs.createReadStream(msg.work.replace('FASTTermEvent', 'FASTEvent'))
        ...
        if (msg.work.search('FASTFormGenre') > -1) type = 'Form'
        if (msg.work.search('FASTTermEvent') > -1) type = 'Event'
        ...
        if (msg.work.search('FASTCorporate') > -1 || msg.work.search('FASTEvent') > -1 || msg.work.search('FASTPersonal') > -1) {
          ...
          _(streamParser)
            ...
            .done(function () {
              ...
              var skipViaf = (msg.work.search('FASTTermEvent') > -1)
              _(allInsert)
                .map((insert) => {
                  if (insert.sameAsViaf.length > 0 && skipViaf) {
                    return ''
                  }
                  if (!insert.fast) {
                    console.log('Bad Fast Obj:', insert)
                    return ''
                  }
                  return insert
                })
